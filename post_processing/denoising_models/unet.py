...
class UNet(nn.Module):
    ...
    def __init__(self, num_input_channels=3, num_output_channels=3, 
        ...
        self.start = unetConv2(
            num_input_channels,
            filters[0] - num_input_channels if concat_x else filters[0],
            norm_layer,
            need_bias,
            pad,
        )
        ...
        self.down1 = unetDown(
            filters[0],
            filters[1] - num_input_channels if concat_x else filters[1],
            norm_layer,
            need_bias,
            pad,
        )
        self.down2 = unetDown(
            filters[1],
            filters[2] - num_input_channels if concat_x else filters[2],
            norm_layer,
            need_bias,
            pad,
        )
        self.down3 = unetDown(
            filters[2],
            filters[3] - num_input_channels if concat_x else filters[3],
            norm_layer,
            need_bias,
            pad,
        )
        self.down4 = unetDown(
            filters[3],
            filters[4] - num_input_channels if concat_x else filters[4],
            norm_layer,
            need_bias,
            pad,
        )
        ...
        if self.more_layers > 0:
            ...
            self.more_ups = [
                unetUp(
                    filters[4], upsample_mode, need_bias, pad, same_num_filt=True
                )
                for _ in range(self.more_layers)
            ]
    ...
    def forward(self, inputs):
        ...
        downs.extend(down(downs[-1]) for _ in range(4 + self.more_layers))
...
class unetUp(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        return self.conv(torch.cat([in1_up, inputs2_], 1))
