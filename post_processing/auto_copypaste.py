...
def detect_keypoints(frame, filename):
    ...
    if len(dets) == 1:
        ...
        print(f"Input an integer to choose one face: {dets}")
...
for vid_img, base_img in zip(vid_list, base_list):
    ...
    vid_top_corr = (
        min(vid_keys[i][1] for i in range(48, 55)) - vid_face_rect_t
    ) / vid_face_rect_height
    ...
    base_top_corr = (
        min(base_keys[i][1] for i in range(48, 55)) - base_face_rect_t
    ) / base_face_rect_height
